...
function openConnection(){
    global $serwer;
    $serwer = mysqli_connect("localhost", "root", "") or exit("Failed to connect to the server");
    $base = 'donut';
	try{
...
function createDatabase(){
	$serwer = mysqli_connect("localhost", "root", "") or exit("Connection to the server failed");
	$base = 'donut';
	
	mysqli_query($serwer, "CREATE DATABASE `$base` DEFAULT CHARACTER SET utf8 COLLATE utf8_polish_ci;") 
	or exit("Error in the query creating the database");
}
...
function createTables(){
	...
	mysqli_query($serwer, $zapytanie) or exit("Error in query: $zapytanie");
